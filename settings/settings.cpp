...
FlipperHTTPSettings::FlipperHTTPSettings(ViewDispatcher **view_dispatcher, void *appContext) : appContext(appContext), view_dispatcher_ref(view_dispatcher)
{
    if (!easy_flipper_set_variable_item_list(&variable_item_list, FlipperHTTPViewSettings,
...
FlipperHTTPSettings::~FlipperHTTPSettings()
{
    ...
    {
        view_dispatcher_remove_view(*view_dispatcher_ref, FlipperHTTPViewSettings);
...
uint32_t FlipperHTTPSettings::callbackToSettings(void *context)
{
    UNUSED(context);
    return FlipperHTTPViewSettings;
}

uint32_t FlipperHTTPSettings::callbackToSubmenu(void *context)
{
    UNUSED(context);
    return FlipperHTTPViewSubmenu;
}

void FlipperHTTPSettings::freeTextInput()
{
    if (text_input && view_dispatcher_ref && *view_dispatcher_ref)
    {
        view_dispatcher_remove_view(*view_dispatcher_ref, FlipperHTTPViewTextInput);
...
bool FlipperHTTPSettings::initTextInput(uint32_t view)
{
...
void FlipperHTTPSettings::settingsItemSelectedCallback(void *context, uint32_t index)
{
    FlipperHTTPSettings *settings = (FlipperHTTPSettings *)context;
    settings->settingsItemSelected(index);
}

bool FlipperHTTPSettings::startTextInput(uint32_t view)
{
    freeTextInput();
    ...
    {
        view_dispatcher_switch_to_view(*view_dispatcher_ref, FlipperHTTPViewTextInput);
        return true;
    }
    else
    {
...
void FlipperHTTPSettings::textUpdated(uint32_t view)
{
    ...
    if (view_dispatcher_ref && *view_dispatcher_ref)
    {
        view_dispatcher_switch_to_view(*view_dispatcher_ref, FlipperHTTPViewSettings);
    }
}

void FlipperHTTPSettings::textUpdatedSsidCallback(void *context)
{
    FlipperHTTPSettings *settings = (FlipperHTTPSettings *)context;
    settings->textUpdated(SettingsViewSSID);
}

void FlipperHTTPSettings::textUpdatedPassCallback(void *context)
{
    FlipperHTTPSettings *settings = (FlipperHTTPSettings *)context;
