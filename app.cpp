...
FlipperHTTPApp::FlipperHTTPApp()
{
    ...
    // Submenu
    if (!easy_flipper_set_submenu(&submenu, FlipperHTTPViewSubmenu,
    ...
    submenu_add_item(submenu, "Run", FlipperHTTPSubmenuRun, submenuChoicesCallback, this);
    submenu_add_item(submenu, "About", FlipperHTTPSubmenuAbout, submenuChoicesCallback, this);
    submenu_add_item(submenu, "Settings", FlipperHTTPSubmenuSettings, submenuChoicesCallback, this);
    ...
    view_dispatcher_switch_to_view(viewDispatcher, FlipperHTTPViewSubmenu);
}
...
FlipperHTTPApp::~FlipperHTTPApp()
{
    ...
    if (submenu)
    {
        view_dispatcher_remove_view(viewDispatcher, FlipperHTTPViewSubmenu);
...
uint32_t FlipperHTTPApp::callbackExitApp(void *context)
{
    UNUSED(context);
    return VIEW_NONE;
}

void FlipperHTTPApp::callbackSubmenuChoices(uint32_t index)
{
    switch (index)
    {
    case FlipperHTTPSubmenuRun:
        ...
        break;
    case FlipperHTTPSubmenuAbout:
        if (!about)
        {
            about = std::make_unique<FlipperHTTPAbout>(&viewDispatcher);
        }
        view_dispatcher_switch_to_view(viewDispatcher, FlipperHTTPViewAbout);
        break;
    case FlipperHTTPSubmenuSettings:
        if (!settings)
        {
...
FuriString *FlipperHTTPApp::httpRequest(
    const char *url,
    HTTPMethod method,
    const char *headers,
    const char *payload)
{
    if (!flipperHttp)
    {
        FURI_LOG_E(TAG, "FlipperHTTPApp::httpRequest: FlipperHTTP is NULL");
        return NULL;
    }
    ...
    {
        FURI_LOG_E(TAG, "FlipperHTTPApp::httpRequest: Failed to send HTTP request");
        return NULL;
    }
...
bool FlipperHTTPApp::httpRequestAsync(
    ...
    const char *payload)
{
    if (!flipperHttp)
    {
        FURI_LOG_E(TAG, "FlipperHTTPApp::httpRequestAsync: FlipperHTTP is NULL");
        return false;
    }
    ...
    if (!flipper_http_request(flipperHttp, method, url, headers, payload))
    {
        FURI_LOG_E(TAG, "FlipperHTTPApp::httpRequestAsync: Failed to send HTTP request");
        return false;
    }
    flipperHttp->state = RECEIVING;
    return true;
...
bool FlipperHTTPApp::loadChar(const char *path_name, char *value, size_t value_size)
{
    Storage *storage = static_cast<Storage *>(furi_record_open(RECORD_STORAGE));
...
bool FlipperHTTPApp::loadFileChunk(const char *filePath, char *buffer, size_t sizeOfChunk, uint8_t iteration)
{
...
void FlipperHTTPApp::runDispatcher()
{
    view_dispatcher_run(viewDispatcher);
}

bool FlipperHTTPApp::saveChar(const char *path_name, const char *value)
{
    Storage *storage = static_cast<Storage *>(furi_record_open(RECORD_STORAGE));
    File *file = storage_file_alloc(storage);
    char file_path[256];
...
bool FlipperHTTPApp::setHttpState(HTTPState state) noexcept
{
    if (flipperHttp)
    {
...
void FlipperHTTPApp::settingsItemSelected(uint32_t index)
{
    if (settings)
    {
...
void FlipperHTTPApp::submenuChoicesCallback(void *context, uint32_t index)
{
    FlipperHTTPApp *app = (FlipperHTTPApp *)context;
    app->callbackSubmenuChoices(index);
}

void FlipperHTTPApp::timerCallback(void *context)
{
    ...
    if (run)
    {
        ...
        else
        {
            ...
            view_dispatcher_switch_to_view(app->viewDispatcher, FlipperHTTPViewSubmenu);
            app->run.reset();
        }
    }
}

void FlipperHTTPApp::viewPortDraw(Canvas *canvas, void *context)
{
    FlipperHTTPApp *app = static_cast<FlipperHTTPApp *>(context);
    furi_check(app);
    auto run = app->run.get();
...
extern "C"
{
    int32_t flipper_http_main(void *p)
    {
